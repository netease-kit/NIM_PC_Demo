...
void AddFriendWindow::InitWindow()
{
	...
	unregister_cb.Add(nim_ui::UserManager::GetInstance()->RegUserPhotoReady(cb));
}
...
bool AddFriendWindow::Search(ui::EventArgs* param)
{
	...
	nim_ui::UserManager::GetInstance()->GetUserInfoWithEffort(std::list<std::string>(1, key), cb);
...
{
	nim_ui::SessionManager::GetInstance()->OpenSessionForm(id_, nim::NIMSessionType::kNIMSessionTypeP2P);
	Close();
	return true;
}
...
bool AddFriendWindow::RemoveFromBlack(ui::EventArgs* args)
{
	nim::User::SetBlackCallback cb = ToWeakCallback([this](int res_code, const std::string& accid, bool opt){
		if (res_code == 200)
		{
			if (nim_ui::UserManager::GetInstance()->GetUserType(id_) == UT_FRIEND)
...
{
	if (nim_ui::LoginManager::GetInstance()->IsEqual(uinfo.account))
	{
	...
	VBox* userinfo_page = static_cast<VBox*>(FindControl(_T("userinfo_page")));
	Button* headimage = (Button*)userinfo_page->FindSubControl(L"headimage");
	headimage->SetBkImage(nim_ui::UserManager::GetInstance()->GetUserPhoto(uinfo.account));
	...
	UserType user_type = nim_ui::UserManager::GetInstance()->GetUserType(id_);
	if (nim_ui::MuteBlackManager::GetInstance()->IsInBlackList(id_))
