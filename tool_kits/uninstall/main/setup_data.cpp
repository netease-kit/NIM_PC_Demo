...
//////////////////
std::wstring g_PRODUCT_NAME = PRODUCT_NAME;
...
int CSetupData::GetDelFileInfoListNum(int step)
{
	...
	if (m_strAryDleFileList.size()==0)
	{
		...
		case 0:
			...
			//删除老的快捷方式
		{
			std::wstring delSTARTMENU($SMPROGRAMS);
			_DelFile(delSTARTMENU + g_PRODUCT_NAME + _T("\\*.*"));
			_DelFile(delSTARTMENU + g_PRODUCT_NAME + _T("\\"));
			_DelFile(delSTARTMENU + g_PRODUCT_NAME + _T(".lnk"));
			std::wstring delDESKTOP($DESKTOP);
			_DelFile(delDESKTOP + g_PRODUCT_NAME + _T(".lnk"));
		...
		case 2:
			//删除用户数据
			_DelFile($LOCALAPPDATA _T("\\") PRODUCT_DOC _T("*.*"));
			_DelFile($LOCALAPPDATA _T("\\") PRODUCT_DOC);
			break;
		}
	}
...
int CSetupData::GetAddRegInfoListNum()
{
	if (m_vecAddRegInfoList.size()==0)
	{
		...
		_WriteRegValue(HKLM, PRODUCT_UNINST_KEY, _T("DisplayName"), g_PRODUCT_NAME);
