...
void Box::HandleMessageTemplate(EventArgs& msg)
{
	...
	if (this == msg.pSender) {
		...
		if (bRet) {
			auto it_event = OnXmlEvent.find(msg.Type);
			if (it_event != OnXmlEvent.end()) {
				bRet = it_event->second(&msg);
			}
			if (weakflag.expired()) {
				return;
			}

			it_event = OnXmlEvent.find(kEventAll);
			if (it_event != OnXmlEvent.end()) {
				bRet = it_event->second(&msg);
			}
			if (weakflag.expired()) {
	...
	if (bRet) {
		auto it_event = OnXmlBubbledEvent.find(msg.Type);
		if (it_event != OnXmlBubbledEvent.end()) {
			bRet = it_event->second(&msg);
		}
		if (weakflag.expired()) {
			return;
		...
		it_event = OnXmlBubbledEvent.find(kEventAll);
		if (it_event != OnXmlBubbledEvent.end()) {
			bRet = it_event->second(&msg);
		}
		if (weakflag.expired()) {
