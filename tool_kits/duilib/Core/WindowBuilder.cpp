...
Box* WindowBuilder::Create(CreateControlCallback pCallback, Window* pManager, Box* pParent, Box* pUserDefinedBox)
{
	...
	if( pManager ) {
		...
		{
			int nAttributes_temp = root.GetAttributeCount();
			for( int i = 0; i < nAttributes_temp; i++ ) {
			...
			if( pManager->GetHWND() ) {
				...
				for( int i = 0; i < nAttributes_temp; i++ ) {
					...
					strValue = root.GetAttributeValue(i);
					if( strName == _T("size") ) {
						LPTSTR pstr_window = NULL;
						int cx = _tcstol(strValue.c_str(), &pstr_window, 10);	ASSERT(pstr_window);
						int cy = _tcstol(pstr_window + 1, &pstr_window, 10);	ASSERT(pstr_window);
						...
						UiRect rcSizeBox;
						LPTSTR pstr_sizebox = NULL;
						rcSizeBox.left = _tcstol(strValue.c_str(), &pstr_sizebox, 10);  ASSERT(pstr_sizebox);
						rcSizeBox.top = _tcstol(pstr_sizebox + 1, &pstr_sizebox, 10);    ASSERT(pstr_sizebox);
						rcSizeBox.right = _tcstol(pstr_sizebox + 1, &pstr_sizebox, 10);  ASSERT(pstr_sizebox);
						rcSizeBox.bottom = _tcstol(pstr_sizebox + 1, &pstr_sizebox, 10); ASSERT(pstr_sizebox);
						pManager->SetSizeBox(rcSizeBox);
					}
					else if( strName == _T("caption") ) {
						UiRect rcCaption;
						LPTSTR pstr_scaption = NULL;
						rcCaption.left = _tcstol(strValue.c_str(), &pstr_scaption, 10);  ASSERT(pstr_scaption);
						rcCaption.top = _tcstol(pstr_scaption + 1, &pstr_scaption, 10);    ASSERT(pstr_scaption);
						rcCaption.right = _tcstol(pstr_scaption + 1, &pstr_scaption, 10);  ASSERT(pstr_scaption);
						rcCaption.bottom = _tcstol(pstr_scaption + 1, &pstr_scaption, 10); ASSERT(pstr_scaption);
						...
						LPTSTR pstr_scaption_roundcorner = NULL;
						int cx = _tcstol(strValue.c_str(), &pstr_scaption_roundcorner, 10);  ASSERT(pstr_scaption_roundcorner);
						int cy = _tcstol(pstr_scaption_roundcorner + 1, &pstr_scaption_roundcorner, 10);    ASSERT(pstr_scaption_roundcorner);
						pManager->SetRoundCorner(cx, cy);
					} 
					else if( strName == _T("mininfo") ) {
						LPTSTR pstr_mininfo = NULL;
						int cx = _tcstol(strValue.c_str(), &pstr_mininfo, 10);  ASSERT(pstr_mininfo);
						int cy = _tcstol(pstr_mininfo + 1, &pstr_mininfo, 10);    ASSERT(pstr_mininfo);
						pManager->SetMinInfo(cx, cy);
					}
					else if( strName == _T("maxinfo") ) {
						LPTSTR pstr_maxinfo = NULL;
						int cx = _tcstol(strValue.c_str(), &pstr_maxinfo, 10);  ASSERT(pstr_maxinfo);
						int cy = _tcstol(pstr_maxinfo + 1, &pstr_maxinfo, 10);    ASSERT(pstr_maxinfo);
						...
						UiRect rc;
						LPTSTR pstr_shadowcorner = NULL;
						rc.left = _tcstol(strValue.c_str(), &pstr_shadowcorner, 10);  ASSERT(pstr_shadowcorner);
						rc.top = _tcstol(pstr_shadowcorner + 1, &pstr_shadowcorner, 10);    ASSERT(pstr_shadowcorner);
						rc.right = _tcstol(pstr_shadowcorner + 1, &pstr_shadowcorner, 10);  ASSERT(pstr_shadowcorner);
						rc.bottom = _tcstol(pstr_shadowcorner + 1, &pstr_shadowcorner, 10); ASSERT(pstr_shadowcorner);
						pManager->SetShadowCorner(rc);
					}
					else if (strName == _T("alphafixcorner") || strName == _T("custom_shadow")) {
						UiRect rc;
						LPTSTR pstr_temp = NULL;
						rc.left = _tcstol(strValue.c_str(), &pstr_temp, 10);  ASSERT(pstr_temp);
						rc.top = _tcstol(pstr_temp + 1, &pstr_temp, 10);    ASSERT(pstr_temp);
						rc.right = _tcstol(pstr_temp + 1, &pstr_temp, 10);  ASSERT(pstr_temp);
						rc.bottom = _tcstol(pstr_temp + 1, &pstr_temp, 10); ASSERT(pstr_temp);
