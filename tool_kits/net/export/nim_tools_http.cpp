...
struct HttpRequest
{
	HttpRequest() :
		...
		speed_cb(nullptr),
		speed_cb_user_data(nullptr),
		transfer_cb(nullptr),
		transfer_cb_user_data(nullptr),
		header_map()
	{
	}
	bool is_set_as_post;
	...
	nim_http_request_speed_cb speed_cb;
	const void* speed_cb_user_data;
	nim_http_request_transfer_cb transfer_cb;
	const void* transfer_cb_user_data;
...
void nim_http_uninit()
{
	...
	if (!bIsUnInited)
	{
		...
		net::GlobalMiscThread::GetInstance()->Stop();
	}
}

void nim_http_init_log(const char* log_file_path)
{
	static net::GlobalMiscThread *global_misc_thread = nullptr;

	if (QLogImpl::GetInstance()->GetLogFile().empty())
	{
		std::wstring path = nbase::UTF8ToUTF16(log_file_path);
		QLogImpl::GetInstance()->SetLogFile(path);
#ifdef _DEBUG
		...
		net::GlobalMiscThread::GetInstance()->Start();	
		QLogImpl::GetInstance()->DoClearLog();
	}
}

bool nim_http_is_init_log()
{
	return !QLogImpl::GetInstance()->GetLogFile().empty();
}
...
HttpRequestID nim_http_post_request(HttpRequestHandle request_handle)
{
	...
	net::CurlHttpRequest::SpeedCallback speed_cb;
	if (http_request->speed_cb) {
		speed_cb = nbase::Bind(http_request->speed_cb, http_request->speed_cb_user_data,
			std::placeholders::_1, std::placeholders::_2);
	}

	net::CurlHttpRequest::TransferCallback transfer_cb;
	if (http_request->transfer_cb) {
		transfer_cb = nbase::Bind(http_request->transfer_cb, http_request->transfer_cb_user_data,
			std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4);
	}
	...
	if (!ws_file_path.empty())
	{
		if (http_request->range_start < 0) {
			curl_http_request = new net::CurlHttpRequest(http_request->url, ws_file_path.c_str(),
				completed_cb, progress_cb, speed_cb, transfer_cb, is_post);
		}
		else {
			curl_http_request = new net::CurlHttpRequest(http_request->url, ws_file_path.c_str(),
				http_request->range_start, completed_cb, progress_cb, speed_cb, transfer_cb, is_post);
		}
	}
	else {
		curl_http_request = new net::CurlHttpRequest(http_request->url, content_cb,
			progress_cb, speed_cb, transfer_cb, is_post);
	}
	...
	if (http_request->timeout_ms > 0)
		shared_request->SetTimeout(http_request->timeout_ms);

	if (http_request->low_speed_limit > 0 && http_request->low_speed_time > 0)
		shared_request->SetLowSpeed(http_request->low_speed_limit, http_request->low_speed_time);
...
void nim_http_set_request_speed_cb(HttpRequestHandle request_handle,
	nim_http_request_speed_cb speed_callback, const void* user_data)
{
	HttpRequest* http_request = (HttpRequest*)request_handle;
	http_request->speed_cb = speed_callback;
	http_request->speed_cb_user_data = user_data;
}

void nim_http_set_request_transfer_cb(HttpRequestHandle request_handle,
	nim_http_request_transfer_cb transfer_callback, const void* user_data)
{
	HttpRequest* http_request = (HttpRequest*)request_handle;
	http_request->transfer_cb = transfer_callback;
	http_request->transfer_cb_user_data = user_data;
}
...
{
	HttpRequest* http_request = (HttpRequest*)request_handle;
	http_request->low_speed_limit = low_speed_limit;
	http_request->low_speed_time = low_speed_time;
}
...
const char* const nim_http_get_response_head(HttpRequestID task_id)
{
	net::CurlHttpRequest* request = net::HttpTransThread::GetInstance()->GetRequestByRequestID(task_id);
	if (request != nullptr)
		return request->response_head().c_str();
	return nullptr ;
}
