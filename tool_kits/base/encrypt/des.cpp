...
{
	char* bitsCiphertextAnyLengthTemp = new char[data_base_length_*4];
	char* hexCiphertextAnyLengthTemp = new char[data_base_length_*2];

	int iLen = 0;

	memset(szCiphertextData,0,data_base_length_);
	memset(hexCiphertextAnyLengthTemp,0,data_base_length_*2);
	

	iLen = (((int)strlen(szCipherInBytes)>>2) + ((int)strlen(szCipherInBytes) % 4 == 0 ? 0 : 1))<<4;
	
	memcpy(hexCiphertextAnyLengthTemp,szCipherInBytes,strlen(szCipherInBytes));
	Hex2Bits(hexCiphertextAnyLengthTemp, bitsCiphertextAnyLengthTemp,iLen);
	Bits2Bytes(szCiphertextData,bitsCiphertextAnyLengthTemp,iLen);

	delete[] bitsCiphertextAnyLengthTemp;
	delete[] hexCiphertextAnyLengthTemp;
