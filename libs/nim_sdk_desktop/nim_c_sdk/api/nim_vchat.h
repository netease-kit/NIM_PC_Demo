...
/** @fn bool nim_vchat_start(NIMVideoChatMode mode, const char *json_extension, const void *user_data)
  ...
  * @param[in] json_extension Json string 扩展，kNIMVChatUids成员id列表(必填),其他可选 如{"uids":["uid_temp"],"custom_video":0, "custom_audio":0, "aec_time":70}
  * @param[in] user_data 无效的扩展字段
  * @return bool true 调用成功，false 调用失败可能有正在进行的通话
  */
NIM_SDK_DLL_API bool nim_vchat_start(NIMVideoChatMode mode, const char *json_extension, const void *user_data);
...
/** @fn bool nim_vchat_callee_ack(__int64 channel_id, bool accept, const char *json_extension, const void *user_data)
  ...
  * @param[in] json_extension 扩展的json string,接起时有效 参数可选 如{"custom_video":0, "custom_audio":0, "aec_time":70}
  * @param[in] user_data 无效的扩展字段
