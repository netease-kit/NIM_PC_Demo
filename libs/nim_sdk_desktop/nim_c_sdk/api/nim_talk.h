...
{
#endif
/** @fn void nim_talk_send_msg(const char *json_msg, const char *json_extension, nim_http_up_prg_cb_func prg_cb, const void *prg_user_data)
  * 发送消息
  * @param[in] json_msg		消息体Json string (Keys SEE MORE `nim_talk_def.h` 『消息结构 Json Keys』)
  * @param[in] json_extension json扩展参数（备用，目前不需要）
  * @param[in] prg_cb		上传进度的回调函数, 如果发送的消息里包含了文件资源，则通过此回调函数通知上传进度, nim_sysmsg_receive_cb_func回调函数定义见nim_http_def.h
  * @param[in] prg_user_data APP的自定义用户数据，SDK只负责传回给回调函数prg_cb，不做任何处理！
  * @return void 无返回值
  */
NIM_SDK_DLL_API void nim_talk_send_msg(const char *json_msg, const char *json_extension, nim_http_up_prg_cb_func prg_cb, const void *prg_user_data);
...
/** @fn void nim_talk_send_custom_sysmsg(const char *json_msg, const char *json_extension)
  * 发送透传消息（统一以系统消息的形式通知）
  * @param[in] json_msg		消息体Json string (Keys SEE MORE `nim_sysmsg_def.h` 『系统消息结构 Json Keys』)
  * @param[in] json_extension json扩展参数（备用，目前不需要）
  * @return void 无返回值
  */
NIM_SDK_DLL_API void nim_talk_send_custom_sysmsg(const char *json_msg, const char *json_extension);
...
/** @fn void nim_talk_reg_custom_sysmsg_arc_cb(const char *json_extension, nim_custom_sysmsg_arc_cb_func cb, const void *user_data)
  ...
  * @return void 无返回值
  */
NIM_SDK_DLL_API void nim_talk_reg_custom_sysmsg_arc_cb(const char *json_extension, nim_custom_sysmsg_arc_cb_func cb, const void *user_data);
#ifdef __cplusplus
};
#endif //__cplusplus
