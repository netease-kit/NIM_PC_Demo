...
/** 
*	@brief NIMSignalingNotityInfo 的宏定义基类
*/
#define NIMSignalingNotityInfoBase \
...
/** @typedef void(*nim_signaling_notify_cb_func)(const struct NIMSignalingNotityInfo* notify_info, const void *user_data)
* @brief 事件回调函数，用于在线通知和多端同步通知
* @param[out] notify_info 事件回调的信息指针，根据NIMSignalingNotityInfo.event_type_指向对应NIMSignalingNotityInfo扩展，如NIMSignalingNotityInfoAccept
* @param[out] user_data APP的自定义用户数据，SDK只负责传回给回调函数cb，不做任何处理！
* @return void 无返回值
*/
typedef void(*nim_signaling_notify_cb_func)(const struct NIMSignalingNotityInfo* notify_info, const void *user_data);

/** @typedef void(*nim_signaling_notify_list_cb_func)(const struct NIMSignalingNotityInfo** info_list, int32_t size, const void *user_data)
* @brief 事件回调函数，用于离线通知
* @param[out] info_list 事件回调的信息NIMSignalingNotityInfo指针的数组，根据NIMSignalingNotityInfo.event_type_指向对应NIMSignalingNotityInfo扩展，如NIMSignalingNotityInfoAccept
