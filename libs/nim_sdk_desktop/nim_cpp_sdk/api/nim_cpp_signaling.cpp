...
#include "nim_signaling.h"
	...
	static void OnNotifyCallback(const struct NIMSignalingNotityInfo* notify_info, const void *user_data)
	{
		CallbackProxy::DoSafeCallback<Signaling::SignalingNotifyCallback>(user_data, [=](const Signaling::SignalingNotifyCallback& cb){
			CallbackProxy::Invoke(cb, SignalingNotityInfo::GetSharedPtr(notify_info));
		}, false);
	}
	static void OnNotifyListCallback(const struct NIMSignalingNotityInfo** info_list, int32_t size, const void *user_data)
	{
		CallbackProxy::DoSafeCallback<Signaling::SignalingNotifyListCallback>(user_data, [=](const Signaling::SignalingNotifyListCallback& cb){
			std::list<std::shared_ptr<SignalingNotityInfo>> post_info;
			if (info_list)
			{
				for (int32_t i = 0; i < size; ++i)
				{
					post_info.push_back(SignalingNotityInfo::GetSharedPtr(info_list[i]));
				}
			}
