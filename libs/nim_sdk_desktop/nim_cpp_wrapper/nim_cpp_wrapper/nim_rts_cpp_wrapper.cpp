...
void StartChannelCallbackWrapper(int code, const char *session_id, int channel_type, const char* uid, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIThread(std::bind((*cb_pointer), (nim::NIMResCode)code, PCharToString(session_id), channel_type, PCharToString(uid)));
		}
		delete cb_pointer;
	}
}
void StartNotifyCallbackWrapper(const char *session_id, int channel_type, const char* uid, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIThread(std::bind((*cb_pointer), PCharToString(session_id), channel_type, PCharToString(uid), custom_info));
		}
	}
}
void AckCallbackWrapper(int code, const char *session_id, int channel_type, bool accept, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIThread(std::bind((*cb_pointer), (nim::NIMResCode)code, PCharToString(session_id), channel_type, accept));
		}
		delete cb_pointer;
	}
}
void AckNotifyCallbackWrapper(const char *session_id, int channel_type, bool accept, const char* uid, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIThread(std::bind((*cb_pointer), PCharToString(session_id), channel_type, accept, PCharToString(uid)));
		}
	}
}
void SyncAckNotifyCallbackWrapper(const char *session_id, int channel_type, bool accept, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIThread(std::bind((*cb_pointer), PCharToString(session_id), channel_type, accept));
		}
	}
}
void ConnectNotifyCallbackWrapper(const char *session_id, int channel_type, int code, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIThread(std::bind((*cb_pointer), PCharToString(session_id), channel_type, code, PCharToString(json_extension)));
		}
	}
}
...
void HangupCallbackWrapper(int code, const char *session_id, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIThread(std::bind((*cb_pointer), (nim::NIMResCode)code, PCharToString(session_id)));
		}
		delete cb_pointer;
	}
}
...
void ControlCallbackWrapper(int code, const char *session_id, const char *info, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIThread(std::bind((*cb_pointer), (nim::NIMResCode)code, PCharToString(session_id), PCharToString(info)));
		}
		delete cb_pointer;
	}
}
void ControlNotifyCallbackWrapper(const char *session_id, const char *info, const char* uid, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			PostTaskToUIThread(std::bind((*cb_pointer), PCharToString(session_id), PCharToString(info), PCharToString(uid)));
		}
	}
}
void RecDataCallbackWrapper(const char *session_id, int channel_type, const char* uid, const char* data, unsigned int size, const char *json_extension, const void *user_data)
{
	if (user_data)
	{
		...
		if (*cb_pointer)
		{
			...
			PostTaskToUIThread(std::bind((*cb_pointer), PCharToString(session_id), channel_type, PCharToString(uid), data_temp));
		}
	}
}
...
void Rts::StartChannel(int channel_type, const std::string& uid, const std::string& apns, const std::string& custom_info, const StartChannelCallback& cb)
{
	...
	return NIM_SDK_Get_Function(nim_rts_start)(channel_type, uid.c_str(), json.c_str(), &StartChannelCallbackWrapper, cb_pointer);
}
...
void Rts::SetStartNotifyCb(const StartNotifyCallback& cb)
{
	...
	return NIM_SDK_Get_Function(nim_rts_set_start_notify_cb_func)(&StartNotifyCallbackWrapper, g_start_notify_cb_pointer);
}
...
void Rts::Ack(const std::string& session_id, int channel_type, bool accept, const AckCallback& cb)
{
	...
	Json::FastWriter fs;
	json = fs.write(values_temp);
	return NIM_SDK_Get_Function(nim_rts_ack)(session_id.c_str(), channel_type, accept, json.c_str(), &AckCallbackWrapper, cb_pointer);
}
...
void Rts::SetAckNotifyCb(const AckNotifyCallback& cb)
{
	...
	return NIM_SDK_Get_Function(nim_rts_set_ack_notify_cb_func)(&AckNotifyCallbackWrapper, g_ack_notify_cb_pointer);
}
...
void Rts::SetSyncAckNotifyCb(const SyncAckNotifyCallback& cb)
{
	...
	return NIM_SDK_Get_Function(nim_rts_set_sync_ack_notify_cb_func)(&SyncAckNotifyCallbackWrapper, g_syncack_notify_cb_pointer);
}
...
void Rts::SetConnectNotifyCb(const ConnectNotifyCallback& cb)
{
	...
	return NIM_SDK_Get_Function(nim_rts_set_connect_notify_cb_func)(&ConnectNotifyCallbackWrapper, g_connect_notify_cb_pointer);
}
...
void Rts::SetMemberChangeCb(const MemberNotifyCallback& cb)
{
	...
	return NIM_SDK_Get_Function(nim_rts_set_member_change_cb_func)(&MemberNotifyCallbackWrapper, g_member_notify_cb_pointer);
}
...
void Rts::Control(const std::string& session_id, const std::string& info, const ControlCallback& cb)
{
	...
	return NIM_SDK_Get_Function(nim_rts_control)(session_id.c_str(), info.c_str(), "", &ControlCallbackWrapper, cb_pointer);
}
...
void Rts::SetControlNotifyCb(const ControlNotifyCallback& cb)
{
	...
	return NIM_SDK_Get_Function(nim_rts_set_control_notify_cb_func)(&ControlNotifyCallbackWrapper, g_control_notify_cb_pointer);
}

//NIM 设置音视频会话类型
void Rts::SetVChatMode(const std::string& session_id, int mode)
{
	return NIM_SDK_Get_Function(nim_rts_set_vchat_mode)(session_id.c_str(), mode, "");
}
...
void Rts::Hangup(const std::string& session_id, const HangupCallback& cb)
{
	...
	return NIM_SDK_Get_Function(nim_rts_hangup)(session_id.c_str(), "", &HangupCallbackWrapper, cb_pointer);
}
...
void Rts::SetHangupNotifyCb(const HangupNotifyCallback& cb)
{
	...
	return NIM_SDK_Get_Function(nim_rts_set_hangup_notify_cb_func)(&HangupNotifyCallbackWrapper, g_hangup_notify_cb_pointer);
}
...
{
	return NIM_SDK_Get_Function(nim_rts_send_data)(session_id.c_str(), channel_type, data.c_str(), data.size(), "");
}
...
void Rts::SetRecDataCb(const RecDataCallback& cb)
{
	...
	return NIM_SDK_Get_Function(nim_rts_set_rec_data_cb_func)(&RecDataCallbackWrapper, g_rec_data_cb_pointer);
}
