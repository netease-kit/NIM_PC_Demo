...
#include "nim_vchat_cpp_wrapper.h"
...
typedef void(*nim_vchat_add_device_status_cb)(nim::NIMDeviceType type, nim_vchat_device_status_cb_func cb);
typedef void(*nim_vchat_remove_device_status_cb)(nim::NIMDeviceType type);
typedef void(*nim_vchat_set_audio_data_cb)(bool capture, nim_vchat_audio_data_cb_func cb);
typedef void(*nim_vchat_set_video_data_cb)(bool capture, nim_vchat_video_data_cb_func cb);
...
{
	return NIM_SDK_Get_Function(nim_vchat_init)(json_info.c_str());
}
//NIM vchat清理
void VChat::Cleanup()
{
	NIM_SDK_Get_Function(nim_vchat_cleanup)("");
}
...
{
	NIM_SDK_Get_Function(nim_vchat_enum_device_devpath)(type, "", cb, nullptr);
}
...
{
	NIM_SDK_Get_Function(nim_vchat_start_device)(type, device_path.c_str(), fps, "", cb, nullptr);
}
//结束设备
void VChat::EndDevice(nim::NIMDeviceType type)
{
	NIM_SDK_Get_Function(nim_vchat_end_device)(type, "");
}
...
{
	NIM_SDK_Get_Function(nim_vchat_add_device_status_cb)(type, cb);
}
//移除设备监听（摄像头和麦克风）
void VChat::RemoveDeviceStatusCb(nim::NIMDeviceType type)
{
	NIM_SDK_Get_Function(nim_vchat_remove_device_status_cb)(type);
}
...
{
	NIM_SDK_Get_Function(nim_vchat_set_audio_data_cb)(capture, cb);
}
...
{
	NIM_SDK_Get_Function(nim_vchat_set_video_data_cb)(capture, cb);
}
...
{
	NIM_SDK_Get_Function(nim_vchat_set_audio_volumn)(volumn, capture);
}
//capture=true	获取麦克风音量
//capture=false	获取播放音量（采集音频播放的声音由麦克风音频调节，这里只调节通话对方的音量）
unsigned char VChat::GetAudioVolumn(bool capture)
{
	return NIM_SDK_Get_Function(nim_vchat_get_audio_volumn)(capture);
}
//自动调节麦克风音量
void VChat::SetAudioInputAutoVolumn(bool auto_volumn)
{
	NIM_SDK_Get_Function(nim_vchat_set_audio_input_auto_volumn)(auto_volumn);
}
//是否自动调节麦克风音量
bool VChat::GetAudioInputAutoVolumn()
{
	return NIM_SDK_Get_Function(nim_vchat_get_audio_input_auto_volumn)();
}
...
{
	NIM_SDK_Get_Function(nim_vchat_set_cb_func)(cb);
}
//启动通话
//json_info 见nim_vchat_def.h
bool VChat::Start(NIMVideoChatMode mode, const std::string& json_info)
{
	return NIM_SDK_Get_Function(nim_vchat_start)(mode, json_info.c_str(), nullptr);
}
//设置通话模式
bool VChat::SetTalkingMode(NIMVideoChatMode mode, const std::string& json_extension)
{
	return NIM_SDK_Get_Function(nim_vchat_set_talking_mode)(mode, json_extension.c_str());
}
//回应邀请
bool VChat::CalleeAck(unsigned __int64 channel_id, bool accept)
{
	return NIM_SDK_Get_Function(nim_vchat_callee_ack)(channel_id, accept, "", nullptr);
}
//通话控制
bool VChat::Control(unsigned __int64 channel_id, NIMVChatControlType type)
{
	return NIM_SDK_Get_Function(nim_vchat_control)(channel_id, type, "", nullptr);
}
//结束通话
void VChat::End()
{
	NIM_SDK_Get_Function(nim_vchat_end)("");
}
