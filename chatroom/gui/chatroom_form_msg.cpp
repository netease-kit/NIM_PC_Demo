...
namespace nim_chatroom
{
    ...
    void ChatroomForm::AddMsgItem(const ChatRoomMessage& result, bool is_history, bool first_msg_each_batch)
    {
        ...
        else if (result.msg_type_ == kNIMChatRoomMsgTypeNotification)
        {
            ...
            if (reader.parse(result.msg_attach_, json))
            {
                ...
                if (nim_json_reader.parse(json.toStyledString(), nim_json_values))
                {
                    ChatRoomNotification notification;
                    notification.ParseFromJsonValue(nim_json_values);
    ...
    void ChatroomForm::SendText(const std::string &text)
    {
        ...
        if (msg_type == kNIMChatRoomMsgTypeText)
            send_msg = ChatRoom::CreateRoomMessage(msg_type, nim::Tool::GetUuid(), text, "", ChatRoomMessageSetting());
        else
            send_msg = ChatRoom::CreateRoomMessage(msg_type, nim::Tool::GetUuid(), bot.ToJsonString(), text, ChatRoomMessageSetting());
    ...
    {
        std::string send_msg = ChatRoom::CreateRoomMessage(kNIMChatRoomMsgTypeCustom, nim::Tool::GetUuid(), attach, "", ChatRoomMessageSetting());
        ChatRoom::SendMsg(room_id_, send_msg);
    }
    ...
    void ChatroomForm::SendImage(const std::wstring &src)
    {
        ...
        nim::NOS::UploadResource(utf8, [this, img, weak_flag](int res_code, const std::string& url) {
            if (!weak_flag.expired() && res_code == nim::kNIMResSuccess)
            {
                ...
                new_img.url_ = url;
                std::string send_msg = ChatRoom::CreateRoomMessage(kNIMChatRoomMsgTypeImage, nim::Tool::GetUuid(), new_img.ToJsonString(), "", ChatRoomMessageSetting());
                ChatRoom::SendMsg(room_id_, send_msg);
            }
        });
    }
