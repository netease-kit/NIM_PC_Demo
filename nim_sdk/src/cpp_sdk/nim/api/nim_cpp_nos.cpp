...
typedef void(*nim_nos_safeurl_to_originurl)(const char *safe_url, const char *json_extension, nim_nos_safeurl_to_originurl_cb_func callback_result, const void *prg_user_data);
#else
#include "nim_nos.h"
#endif
...
{
	CallbackProxy::DoSafeCallback<NOS::SafeURLToOriginURLCallback>(user_data, [=](const NOS::SafeURLToOriginURLCallback& cb) {
...
void NOS::InitConfig(const InitNosConfigParam& param, const InitNosResultCallback& cb)
{
	...
	NIM_SDK_GET_FUNC(nim_nos_init_config)(nim_cpp_wrapper_util::Json::FastWriter().write(json_tags).c_str(), CallbackInitConfig, param.GetExtension().c_str(), (new InitNosResultCallback(cb)));
}
